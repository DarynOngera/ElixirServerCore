defmodule Core.JobStore.SQLiteTest do
  ...
  setup do
    ...
    {:ok, store_state: state}
  end

  describe "init/1" do
    test "creates the jobs table and index", %{store_state: state} do
      # init was called in setup; just verify we can insert
      job = %Job{payload: %{"task" => "init_test"}, inserted_at: DateTime.utc_now()}
      assert {:ok, %Job{id: id}} = SQLite.insert_job(state, job)
      assert is_integer(id) and id > 0
    end
  end

  describe "insert_job/2" do
    test "assigns an auto-increment id", %{store_state: state} do
      job1 = %Job{payload: %{"n" => 1}, inserted_at: DateTime.utc_now()}
      job2 = %Job{payload: %{"n" => 2}, inserted_at: DateTime.utc_now()}

      {:ok, job1} = SQLite.insert_job(state, job1)
      {:ok, job2} = SQLite.insert_job(state, job2)

      assert job2.id == job1.id + 1
    end

    test "persists all fields", %{store_state: state} do
      ...
      {:ok, inserted} = SQLite.insert_job(state, job)
      {:ok, fetched} = SQLite.get_job(state, inserted.id)
  ...
  describe "get_job/2" do
    test "returns not_found for missing job", %{store_state: state} do
      assert SQLite.get_job(state, 999_999) == {:error, :not_found}
    end
  end

  describe "update_job/3" do
    test "partially updates a job", %{store_state: state} do
      job = %Job{payload: %{"t" => 1}, inserted_at: DateTime.utc_now()}
      {:ok, job} = SQLite.insert_job(state, job)

      :ok = SQLite.update_job(state, job.id, status: :running, attempt: 1)
      {:ok, updated} = SQLite.get_job(state, job.id)
    ...
    test "stores result as json", %{store_state: state} do
      job = %Job{payload: %{"t" => 1}, inserted_at: DateTime.utc_now()}
      {:ok, job} = SQLite.insert_job(state, job)

      :ok = SQLite.update_job(state, job.id, result: %{"status" => "ok"})
      {:ok, updated} = SQLite.get_job(state, job.id)
  ...
  describe "list_jobs/2" do
    test "filters by status", %{store_state: state} do
      for i <- 1..3 do
        job = %Job{payload: %{"n" => i}, inserted_at: DateTime.utc_now(), status: :queued}
        {:ok, _} = SQLite.insert_job(state, job)
      end

      # Mark one as running
      [first | _] = SQLite.list_jobs(state, status: :queued)
      :ok = SQLite.update_job(state, first.id, status: :running)

      queued = SQLite.list_jobs(state, status: :queued)
      running = SQLite.list_jobs(state, status: :running)

      assert length(queued) == 2
      assert length(running) == 1
    end

    test "returns all jobs when no filter given", %{store_state: state} do
      job = %Job{payload: %{}, inserted_at: DateTime.utc_now()}
      {:ok, _} = SQLite.insert_job(state, job)

      assert length(SQLite.list_jobs(state, [])) == 1
    end
  end

  describe "cleanup/2" do
    test "removes old done/failed jobs", %{store_state: state} do
      now = DateTime.utc_now()
      ...
      {:ok, old_done} = SQLite.insert_job(state, old_done)
      :ok = SQLite.update_job(state, old_done.id, status: :done, finished_at: old_done.finished_at)
      ...
      {:ok, fresh_done} = SQLite.insert_job(state, fresh_done)
      :ok = SQLite.update_job(state, fresh_done.id, status: :done, finished_at: fresh_done.finished_at)

      SQLite.cleanup(state, max_age_days: 7)

      assert SQLite.get_job(state, old_done.id) == {:error, :not_found}
      assert {:ok, _} = SQLite.get_job(state, fresh_done.id)
    end
  end
